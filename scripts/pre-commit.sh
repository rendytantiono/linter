...
# Get staged changes
CHANGED=$(git diff --staged --dirstat=files --diff-filter=ACM | awk '{print $2}' | grep -vE "cmd|srcClean")

. $(dirname "$0")/utils/chalk.sh

function main() {
  bannerInfo "Running linter"
  if ! runLinter; then
    echo
  ...
  echo
  bannerInfo "Running tests on committed changes"
  if ! runTests; then
...
function runLinter() {
  ...
  for dir in $CHANGED; do
    ...
    if golangci-lint run "$dir" --new-from-rev=HEAD~; then
...
function runTests() {
  ...
  for changed in $CHANGED; do
    for dir in $(find $changed -maxdepth 10 -not -path './.git*' -not -path '*/_*' -not -path './vendor*' -type d); do
      # Ignore directory if there are no Go files
      if ! ls "${dir}"/*.go &>/dev/null; then continue; fi
